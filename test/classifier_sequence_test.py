...
class TestClassifierSequence:
    ...
    def test_len(self, icon_builder, num_icons, expected_len):
        classifier_sequence = _get_sequence(icon_builder, num_icons=num_icons, batch_size=3)

        assert len(classifier_sequence) == expected_len

    def test_all_batches_have_full_batch_size(self, icon_builder):
        batches = []
        batch_size = 2
        classifier_sequence = _get_sequence(icon_builder, batch_size=batch_size, num_icons=3)
...
def _get_sequence(icon_builder, batch_size, num_icons, num_categories=1):
    ...
    classifier_sequence = ClassifierSequence(icon_builder.input_dir, batch_size, 192)
    return classifier_sequence
